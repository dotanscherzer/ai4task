...
כללים:
- שאל שאלות קצרות, ברורות ורלוונטיות. הימנע משאלות מורכבות או ארוכות מדי. כל שאלה צריכה להיות ממוקדת וקלה להבנה.
- אם השאלה לא רלוונטית לנושא הנוכחי, דלג עליה או עבור לשאלה הבאה.
- שאלה אחת בכל פעם. קצר ומקצועי.
- **חשוב מאוד**: אל תחזור על התשובה של המשתמש. תמיד שאל שאלה חדשה או המשך לשאלה הבאה. ה-bot_message חייב להיות שאלה חדשה או הודעה מעבר, לא חזרה על מה שהמשתמש אמר.
- אם התשובה קצרה מדי או לא מספיק מפורטת, שאל שאלת המשך (FOLLOW_UP) אחת בלבד.
...
- **חשוב מאוד - covered_points**: ה-covered_points חייב להכיל רק נקודות למידה/תובנות/סיכומים מהתשובות, לא שאלות! דוגמאות טובות: "האתגר מתבטא בפספוס יכולות במערכת החדשה", "צוות ארכיטקטים הוא הקבוצה המושפעת ביותר". דוגמאות שגויות (אל תכניס אותן): "איך האתגר מתבטא?", "מי הקבוצה המושפעת?" - אלה שאלות, לא נקודות למידה!
...
החזר JSON בלבד בפורמט:
{
  ...
  "covered_points": ["נקודת למידה/תובנה מהתשובות - לא שאלה!"],
...
export class LLMService {
  ...
  private buildUserPrompt(
    ...
    prompt += `**הוראה חשובה**: המשתמש ענה: "${managerMessage}". שאל שאלה חדשה או המשך לשאלה הבאה. אל תחזור על התשובה של המשתמש. ה-bot_message חייב להיות שאלה חדשה או הודעה מעבר, לא חזרה על מה שהמשתמש אמר.\n\n`;
...
`;
    ...
    return `צור 3-4 שאלות מנחות בעברית עבור ריאיון.
